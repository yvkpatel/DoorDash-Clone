...
class RestaurantGateway:
    ...
    def getRestaurantFromID(self, restID):
        ...
        restaurant = Restaurant(response['unique_id'], response['username'], response['name'], response['orders'],
                                response['menu_id'], response['category'], response['availability'], response['hour_open'],
                                response['hour_close'], response['address'], response['postal'])
        return restaurant
    ...
    def getAvailableRestaurants(self):
        ...
        for r in response:
            restaurant = Restaurant(r['unique_id'], r['username'], r['name'], r['orders'],
                                    r['menu_id'], r['category'], r['availability'], r['hour_open'],
                                    r['hour_close'], r['address'], r['postal'])
            restaurants.append(restaurant)
        return restaurants
